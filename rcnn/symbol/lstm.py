...
def _lstm(num_hidden, indata, prev_state, param, seqidx, layeridx):
    ...
    in_gate = mx.sym.hard_sigmoid(slice_gates[0])
    in_transform = mx.sym.Activation(slice_gates[1], act_type="tanh")
    #forget_gate = mx.sym.Activation(slice_gates[2], act_type="sigmoid")
    forget_gate = mx.sym.hard_sigmoid(slice_gates[2])
    #out_gate = mx.sym.Activation(slice_gates[3], act_type="sigmoid")
    out_gate = mx.sym.hard_sigmoid(slice_gates[3])
...
def lstm(net, num_lstm_layer, num_hidden, seq_length):
    ...
    slices_net = mx.sym.SliceChannel(data=net, axis=3, num_outputs=seq_length, squeeze_axis=1) # bz x features x 1 x time_step
